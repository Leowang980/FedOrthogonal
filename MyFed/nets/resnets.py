...
class ResNet(nn.Module):
    def __init__(self, block, args, model_rate, num_blocks=[2, 2, 2, 2]):
        ...
        self.linear_projector=nn.Linear(int(np.ceil(model_rate*hidden_size[-1])), hidden_size[-1])
        self.orthogonal_projector=nn.utils.parametrizations.orthogonal(nn.Linear(int(np.ceil(model_rate*hidden_size[-1])), hidden_size[-1]))
