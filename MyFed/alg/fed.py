...
from alg.distill import hetero_feature_logit_distillation
...
class FedAvg(object):
    ...
    def train(self):
        ...
        for round_idx in range(start_round, self.args.communication_round):
            ...
            self.model.load_state_dict(avg_weight, strict=False)
...
class HeteroFL(object):
    ...
    def train(self):
        ...
        for round_idx in range(start_round, self.args.communication_round):
            ...
            #print(global_weight)
            self.global_model.load_state_dict(global_weight, strict=False)
...
class Fed_Distill_homo(object):
    ...
    def train(self):
        ...
        for round_idx in range(start_round, self.args.communication_round):
            ...
            self.global_model.load_state_dict(avg_weight, strict=False)
            acc= global_test(self.args, self.global_model, self.dataloader_test_global)
            end_time=time.time()
            longing_time=end_time-start_time
            all_acc.append(acc)
        ...
        for round_idx in range(start_round, self.args.communication_round):
            ...
            if round_idx >= self.args.warmup_round and round_idx<45:
                if m==1:
                    print('开始蒸馏')
                    m-=1
                if self.args.method in ['FedOrthogonal', 'FedLinear']:
                    avg_weight=hetero_feature_distillation(self.args, self.global_model, self.model_rate, total_num, self.client_list, local_param, avg_weight, 
                                                            selected_client, self.train_len_dict, self.dataloader_distill)
                elif self.args.method in ['FedOrthogonalLogit', 'FedLinearLogit']:
                    avg_weight=hetero_feature_logit_distillation(self.args, self.global_model, self.model_rate, total_num, self.client_list, local_param, avg_weight, 
                                                                selected_client, self.train_len_dict, self.dataloader_distill)
